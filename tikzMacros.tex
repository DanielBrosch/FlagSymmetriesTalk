...
\newcommand{\GraphPoly}[2]{\mathbb{R}\left[#1,#2\right]}
...
\newcommand{\FlagGraph}[3][very thick,white,scale=1.45]{
\ifnum#2=2%
    %\tikz[baseline=(tmp1.south)]{
    \tikz[baseline=0pt]{%(tmp1.center)]{
        \node[#1, circle,inner sep=2pt,fill] (tmp1) at (0,-8pt){};
        \node[#1,circle,inner sep=2pt,fill] (tmp2) at (0,24pt){};
        \ifx#3\empty%
        \else
            \draw[#1] (0,-4pt) -- (0,12pt);
        \fi
...
\newcommand{\FlagGraphNumb}[4][very thick, white, scale=1.45]{
\ifnum#2=2%
    \tikz[baseline=3pt]{%(tmp2.center)]{
        ...
        \fi
        \ifnum#4>0
          \node[#1, circle,inner sep=0.5pt, fill=black, draw=white] (tmp1) at (0,32pt){\footnotesize 1};
        \else
          \node[#1, circle,inner sep=2pt, fill] (tmp1) at (0,32pt){};
        \fi
        \ifnum#4>1
          \node[#1,circle,inner sep=0.5pt, fill=black, draw=white] (tmp2) at (0,-8pt){\footnotesize 2};
        \else 
          \node[#1,circle,inner sep=2pt, fill] (tmp2) at (0,-8pt){};
        \fi
    }
\else%
    \tikz[baseline=-2pt]{
        ...
        \foreach \X in {1,...,#2}{
            ...
            \ifnum#4>\result
              \node [#1,circle=black,inner sep=0.5pt, fill=black, draw=white] at (tmp.corner \X){\footnotesize $\X$};
            \else 
              \node [#1,circle,inner sep=2pt, fill] at (tmp.corner \X){};
            \fi
        }
